...
import { fetchOgranizerProfile } from '@/lib/redux/slices/OrganizerProfileSlice';
...
export default function OrganizerProfilePage() {
  ...
  const { data: profile, loading } = useAppSelector((state) => state.OrganizerProfile);
  ...
  useEffect(() => {
    dispatch(fetchOgranizerProfile());
  }, [dispatch]);
  ...
  const handleUploadPhoto = async (file: File) => {
    ...
    try {
      setIsUploading(true);
      const token = localStorage.getItem('token');
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/profile/organizer/upload-picture`, {
      method: 'PATCH',
  ...
  try {
    setIsDeleting(true);
    const token = localStorage.getItem('token');
    const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/profile/organizer/delete-picture`, {
      method: 'PATCH',
      headers: { Authorization: `Bearer ${token}` },
    });
...
function EventList({ events }: { events: any[] }) {
  return (
    <div>
      <h2 className="text-lg font-semibold text-yellow-600 mb-2">Event Saya</h2>
      {events?.length > 0 ? (
        <ul className="space-y-3 text-sm">
          {events.map((event) => (
            <li key={event.id} className="border border-gray-200 p-3 rounded-md">
              <div className="flex flex-col">
                <span className="font-medium text-gray-800">{event.name}</span>
                <span className="text-gray-600">
                  {dayjs.utc(event.start_date).format('DD MMM YYYY')} - {dayjs.utc(event.end_date).format('DD MMM YYYY')}
                </span>
              </div>
            </li>
          ))}
        ...
        <p className="text-sm italic text-gray-500">Tidak ada event yang terdaftar.</p>
      )}
    </div>
  );
}